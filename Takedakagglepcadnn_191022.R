...
Gauss_holdout <- function(okayama, tokyo, nagoya, aichi, gifu){
#  okayama = 0.2074
#  tokyo = 0.4016
#  nagoya = 0.0876
#  aichi = 0.5907
...
train_data.t <- train_data[, c(1:(floor(gifu*2500) + 20))]
test_data.t <- test_data.t[, c(1:(floor(gifu*2500) + 20))]
  
build_model <- function() {

    model <- keras_model_sequential() %>%
    layer_dense(units = floor(tokyo*1000) + 3, activation = "relu",
                input_shape = dim(train_data.t)[2]) %>%
...
opt_svm <- BayesianOptimization(Gauss_holdout,bounds=list(okayama=c(0,1),tokyo=c(0,1),nagoya =c(0,1), aichi = c(0,1), gifu = c(0,1)),init_points=15, n_iter=40, acq='ucb', kappa = 2.576 / 2, verbose=TRUE)
...
write.csv(test_predictions.t[1:13732], "C:/Users/withpcabest1009.csv")
